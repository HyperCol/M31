...
void main() {
    ...
    vec3 accumulation = mix(color, clamp(previousColor, minColor, maxColor), vec3(blend));

    #if Camera_Shutter_Speed > 0
    accumulation = mix(accumulation, previousColor, min(1.0 - 1.0 / Camera_FPS, float(Camera_Shutter_Speed) / Camera_FPS));
    #endif
